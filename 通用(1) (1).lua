local base64 = '+EsugAb24h5GH01lIJOUSq3jPamtedDKLTwM7zNBRpic/Q6VWfyFn8xroZ9kYCXv'
...
if _G.ScriptLoad ~= Base64_enc_Args(game:GetService("Players").LocalPlayer.Name .. "/B/123fa98") and Load ~= "被遗弃" then
    game:GetService("Players").LocalPlayer:Kick("识别码验证失败")
    wait(3)
    game:shutdown()
    wait(1)
    kick("(404)")
...
local Tabs = {
	bbh = Window:AddTab("脚本", "user"),
	Main = Window:AddTab("玩家", "house"),
    esp = Window:AddTab("ESP", "eye"),
    lol = Window:AddTab("传送与甩飞", "earth"),
    nima = Window:AddTab("自瞄", "file"),
    nix = Window:AddTab("范围与大头", "moon"),
    xx = Window:AddTab("信息", "sun"),
    qmp = Window:AddTab("反馈与娱乐", "user"),
...
local LocalPlayer = Players.LocalPlayer
local espb = Tabs.esp:AddRightGroupbox("ESP Settings")
...
local function GetColor(player)
    return (ESPSettings.TeamColor and player.Team) and player.Team.TeamColor.Color or Color3.new(1,1,1)
end
...
local function CreateESP(player)
    ...
    local obj = {
        ...
        Ray = Drawing.new("Line")
    }
    
    obj.Highlight.Name = player.Name.."_HL"
    obj.Highlight.FillTransparency = 0.5
    obj.Highlight.OutlineTransparency = 0
    obj.Highlight.Parent = game.CoreGui
...
RunService.RenderStepped:Connect(function()
    for player, obj in pairs(ESPObjects) do
        local valid = player:IsA("Player")
            and player.Character
            and player.Character:FindFirstChild("HumanoidRootPart")
            and player.Character.Humanoid.Health > 0
            
        if not valid or not IsEnemy(player) then
        ...
        obj.Highlight.Enabled = ESPSettings.Box3D
        obj.Highlight.FillColor = color
        obj.Highlight.OutlineColor = color
        
        if head and ESPSettings.PlayerESP then
        ...
        if ESPSettings.NameESP and hrp then
        ...
        if ESPSettings.DistanceESP and LocalPlayer.Character then
            ...
            if localHrp and hrp then
                ...
                obj.Distance.Position = Vector2.new(pos.X, pos.Y + 40)
        ...
        if ESPSettings.HealthESP and hrp then
            local humanoid = char:FindFirstChildOfClass("Humanoid")
            if humanoid then
                local ratio = humanoid.Health / humanoid.MaxHealth
                local pos = Camera:WorldToViewportPoint(hrp.Position)
                obj.Health.Text = "HP: "..math.floor(humanoid.Health)
                obj.Health.Position = Vector2.new(pos.X, pos.Y + 60)
        ...
        if ESPSettings.RayESP and LocalPlayer.Character then
            local localHrp = LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
            if localHrp and hrp then
                local startPos = Camera:WorldToViewportPoint(localHrp.Position)
                local endPos = Camera:WorldToViewportPoint(hrp.Position)
                obj.Ray.From = Vector2.new(startPos.X, startPos.Y)
                obj.Ray.To = Vector2.new(endPos.X, endPos.Y)
                obj.Ray.Color = color
...
espb:AddToggle("PlayerBox", {
    Text = "玩家方框",
    Default = false,
    Callback = function(s)
        ESPSettings.PlayerESP = s
    end
})

espb:AddToggle("3DBox", {
    Text = "3D高光",
    Default = false,
    Callback = function(s)
        ESPSettings.Box3D = s
...
espb:AddToggle("TeamColor", {
    Text = "队伍颜色",
    Default = false,
    Callback = function(s)
        ESPSettings.TeamColor = s
    end
})

espb:AddToggle("FilterEnemy", {
    Text = "仅显示敌人",
    Default = false,
    Callback = function(s)
        ESPSettings.OnlyEnemies = s
    end
})

espb:AddToggle("ShowName", {
    Text = "显示名字",
    Default = false,
    Callback = function(s)
        ESPSettings.NameESP = s
    end
})

espb:AddToggle("ShowDistance", {
    Text = "显示距离",
    Default = false,
    Callback = function(s)
        ESPSettings.DistanceESP = s
    end
})

espb:AddToggle("ShowHealth", {
    Text = "显示血量",
    Default = false,
    Callback = function(s)
        ESPSettings.HealthESP = s
    end
})

espb:AddToggle("ShowRay", {
    Text = "显示射线",
    Default = false,
    Callback = function(s)
        ESPSettings.RayESP = s
    end
})
...
auto:AddInput('SayMessage', {
    Default = '',
    Text = '输入你要说的话',
    Placeholder = '请输入内容',
    Callback = function(txt)
        bai.saymessage = tostring(txt)  
    end
})

auto:AddInput('SayCount', {
    Default = '1',
    Numeric = true,
    Text = '输入说话次数',
    Placeholder = '输入次数',
    Callback = function(txt)
        bai.saycount = math.clamp(tonumber(txt) or 1, 1, 100)  
    end
})


local function SafeSendMessage(msg)
    pcall(function()  
        if ChatService:FindFirstChild("TextChannels").RBXGeneral:SendAsync(msg) then
            return true
        end
    end)
    return false
...
auto:AddButton({
    Text = '发言一次',
    Func = function()
        for i = 1, bai.saycount do
            if not SafeSendMessage(bai.saymessage) then
                    Library:Notify("服务器阻止发送消息", nil, 4590657391)
                break
            end
            task.wait(bai.cooldown)  
        end
    end
})

auto:AddToggle('AutoSpeak', {
    Text = '全自动说话',
    Default = false,
    Callback = function(state)
        bai.autosay = state
        ...
        if state then
            ...
            connection = RunService.Heartbeat:Connect(function()
                if not bai.autosay then 
                    connection:Disconnect()
                    return 
                end
                
                local success = SafeSendMessage(bai.saymessage)
                if not success then
                    bai.autosay = false
                    Library:Notify("自动发言已关闭：消息发送失败", nil, 4590657391)
                end
                task.wait(bai.cooldown)
            end)
        end
...
-- 附加组件：
-- SaveManager（允许您拥有配置系统）
-- ThemeManager（允许您拥有菜单主题系统）
...
-- 将我们的 MenuKeybind 添加到忽略列表
--（您是否希望每个配置都有不同的菜单键？可能不是。）
SaveManager:SetIgnoreIndexes({ "MenuKeybind" })

-- 这样做的用例：
-- 脚本中心可以在全局文件夹中包含主题
-- 并且每个游戏在单独的文件夹中包含游戏配置
ThemeManager:SetFolder("MyScriptHub")
SaveManager:SetFolder("MyScriptHub/specific-game")
SaveManager:SetSubFolder("specific-place") -- 如果游戏内部有多个地方（例如：DOORS）
-- 您可以使用它来单独保存这些位置的配置
-- 此脚本中的路径为：MyScriptHub/specific-game/settings/specific-place
-- [ 这是可选的 ]

-- 在选项卡右侧构建配置菜单
SaveManager:BuildConfigSection(Tabs["UI Settings"])

-- 在左侧构建我们的主题菜单（包含大量内置主题）
-- 注意：您还可以调用 ThemeManager:ApplyToGroupbox 将其添加到特定的组框
ThemeManager:ApplyToTab(Tabs["UI Settings"])

-- 您可以使用 SaveManager:LoadAutoloadConfig() 来加载配置
-- 已被标记为自动加载！
